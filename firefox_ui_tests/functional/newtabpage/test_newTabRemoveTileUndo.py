...
class NewTabPageRemoveSuggestedUndo(NewTabPageTestCase):

    """ Verify that the Undo button restores removed tile """

    def test_remove_suggested_undo(self):
        with self.marionette.using_context('content'):
            ...
            # Undo should bring removed tile back to starting slot
            undo_button = self.get_undo_button()
            self.assertTrue(
                undo_button is not None,
                'FAILED: Undo Button not present after a tile removed')
            self.undo()
            time.sleep(1)
            ...
            self.assertEqual(ending_tiles[2]['cell_text'],
                             ...
                             'FAILED: Tile not undo to original location')
